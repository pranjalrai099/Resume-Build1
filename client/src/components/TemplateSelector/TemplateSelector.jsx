            ...
            <div className='absolute top-full w-xs p-3 mt-2 space-y-3 z-10 bg-white rounded-md border border-gray-200 shadow-sm'>
                {templates.map((template)=>(
                    <div key={template.id} onClick={()=> {
                        if (template.isPremium && !user?.isPremium) return toast.info('Upgrade to Premium to use this layout!');
                        ...
                        {template.isPremium && (
                            <div className='absolute top-2 right-2 flex items-center gap-1 bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full text-[8px] font-bold uppercase'>
                                Premium
                            </div>
                        )}
